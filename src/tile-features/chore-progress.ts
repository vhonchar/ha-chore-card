...
class ChoreProgress extends LitElement {
  static styles = css`
    md-linear-progress {
      ...
      --md-linear-progress-track-height: var(--feature-height, 36px);
      --md-linear-progress-track-shape: var(--feature-height, 36px);
      --md-linear-progress-active-indicator-height: var(--feature-height, 36px);
    }
  ...
  render() {
    ...
    const progress = Math.min(
      1,
      this.stateObj.attributes.counter_state / this.stateObj.attributes.limit
    );
    const statusClass =
      progress >= 1 ? "overdue" : progress > 0.8 ? "warning" : "good";
